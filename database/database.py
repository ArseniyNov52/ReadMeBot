import os
import psycopg2
from psycopg2 import sql # Для безопасной генерации SQL
from psycopg2.extras import RealDictCursor, execute_values # Для удобства и эффективности
from PyPDF2 import PdfReader
import chardet
import json # Для работы с JSONB в chat_history

# --- Настройки подключения к PostgreSQL ---
# Читаем из переменных окружения (предпочтительный способ)
DB_NAME = os.getenv("DB_NAME", "book_reader_db")
DB_USER = os.getenv("DB_USER", "book_reader_user")
DB_PASSWORD = os.getenv("DB_PASSWORD", "Who132853!") # !!! ЗАМЕНИТЕ ИЛИ УСТАНОВИТЕ ПЕРЕМЕННУЮ !!!
DB_HOST = os.getenv("DB_HOST", "localhost")
DB_PORT = os.getenv("DB_PORT", "5432")

# Базовая директория для хранения файлов книг пользователей
USER_BOOKS_BASE_DIR = "books"
# Убедимся, что директория существует при запуске
...
def initialize_database():
    ...
    commands = (
        ...
        CREATE TABLE IF NOT EXISTS reading_state (
            ...
            -- Новые поля:
            is_session BOOLEAN DEFAULT FALSE,             -- Открыта ли книга в данный момент в сессии чтения
            is_opened BOOLEAN DEFAULT FALSE,              -- Открывал ли пользователь книгу хотя бы раз
            update_page INTEGER DEFAULT 0,                -- Последняя страница, до которой обновлен AI контекст
            book_context TEXT,                            -- Сжатый контекст книги для AI
            chat_history JSONB DEFAULT '[]'::jsonb,       -- История чата с AI по этой книге
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            for command in commands:
                # print(f"Выполнение: {command[:100]}...") # Раскомментируйте для детального лога
                cur.execute(command)
            conn.commit()
...
def _get_book_info(cursor, user_id: int, book_name: str) -> dict | None:
    ...
    # RealDictCursor вернет словарь или None
...
def get_book_id_by_name(user_id: int, book_name: str) -> int | None:
    ...
    try:
        ...
        # Используем RealDictCursor для удобства
...
# ------------------------------
# Функции работы с книгами (добавление, получение списка)
# ------------------------------
...
def _calculate_total_pages(full_file_path: str, file_format: str, page_size_txt: int = 1000) -> int | None:
    ...
    Возвращает число страниц или None при ошибке/пустом файле, требующем расчета.
    """
    if not os.path.exists(full_file_path) or os.path.getsize(full_file_path) == 0:
        # print(f"Файл не найден или пуст: {full_file_path}")
        return 0 # Пустая книга имеет 0 страниц

    try:
        if file_format == 'pdf':
            with open(full_file_path, "rb") as book_file:
                reader = PdfReader(book_file)
                # У PdfReader могут быть проблемы с некоторыми файлами, добавим проверку
                try:
                    ...
                    return num_pages
                     ...
                     return None # Возвращаем None, если не можем посчитать
        elif file_format == 'txt':
            total_size = os.path.getsize(full_file_path)
            # Расчет количества блоков: (total_size + page_size - 1) // page_size
        ...
        return None # Ошибка при подсчете
...
def add_book_to_db(user_id: int, book_name: str, full_file_path: str, page_size_txt: int = 1000) -> dict | None:
    ...
    # total_pages может быть None, если не удалось посчитать
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                ...
                FROM books
                WHERE user_id = %s
                ORDER BY book_name ASC; -- Сортировка по имени для удобства клавиатуры
                """,
                (user_id,)
            )
            books = cur.fetchall()
...
# ------------------------------
# Функции работы с состоянием чтения (Основные)
# ------------------------------
...
def update_reading_state(user_id: int, book_id: int, page: int) -> dict | None:
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            # 1. Получить total_pages из таблицы books
            ...
            # 2. Корректируем страницу, если она выходит за пределы
            if total_pages is not None and page >= total_pages:
                page = total_pages - 1 if total_pages > 0 else 0

            # 3. Обновить или вставить состояние чтения (UPSERT для current_page)
            query = sql.SQL("""
            ...
            # print(f"Состояние чтения для user {user_id}, book {book_id} обновлено: стр {updated_page}/{total_pages}")
...
def get_current_page(user_id: int, book_id: int) -> int | None:
    ...
    """
    conn = None
    current_page = 0 # По умолчанию 0, если нет записи
    try:
        conn = get_db_connection()
        if conn is None: return None # Ошибка подключения
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            # Если result is None, значит записи нет, оставляем current_page = 0
...
# ------------------------------
# Функции работы с состоянием чтения (Статусы: is_session, is_opened)
# ------------------------------
...
def set_book_session_status(user_id: int, book_id: int, status: bool) -> bool:
    ...
    try:
        ...
        with conn.cursor() as cur:
            # Убедимся, что запись в reading_state существует (или создадим с page=0)
            cur.execute(
                 """
            ...
            # Теперь обновляем статус сессии
            cur.execute(
                "UPDATE reading_state SET is_session = %s WHERE user_id = %s AND book_id = %s;",
                (status, user_id, book_id)
            )
            conn.commit()
            success = cur.rowcount > 0 # Проверяем, была ли обновлена строка
            # if success: print(f"Статус сессии для user {user_id}, book {book_id} установлен в {status}")
...
def set_book_opened_status(user_id: int, book_id: int, status: bool) -> bool:
    ...
    try:
        ...
        with conn.cursor() as cur:
             ...
             # if success: print(f"Статус is_opened для user {user_id}, book {book_id} установлен в {status}")
...
def get_reading_state_details(user_id: int, book_id: int) -> dict | None:
    ...
    try:
        ...
        details = None # Явно None при ошибке
    finally:
        if conn: conn.close()
    return details

# ------------------------------
# Функции работы с состоянием чтения (AI-контекст и чат)
# ------------------------------
...
def update_ai_context(user_id: int, book_id: int, update_page: int, book_context: str | None) -> bool:
    ...
    try:
        ...
        with conn.cursor() as cur:
             ...
             conn.commit()
             success = cur.rowcount > 0
             # if success: print(f"AI контекст для user {user_id}, book {book_id} обновлен (update_page={update_page}).")
...
def add_chat_message(user_id: int, book_id: int, role: str, content: str) -> bool:
    ...
    try:
        ...
        if conn is None: return False
        with conn.cursor() as cur:
            # Сначала убедимся, что строка существует
            cur.execute(
                 """
                 INSERT INTO reading_state (user_id, book_id, current_page)
                 VALUES (%s, %s, 0) ON CONFLICT (user_id, book_id) DO NOTHING;
            ...
            # Теперь добавляем сообщение в массив
            cur.execute(
                """
                ...
                WHERE user_id = %s AND book_id = %s;
                """,
                (json.dumps(message_dict), user_id, book_id) # || - оператор конкатенации для jsonb
            )
            conn.commit()
            success = cur.rowcount > 0
            # if success: print(f"Сообщение ({role}) добавлено в историю чата user {user_id}, book {book_id}.")

    except (Exception, psycopg2.DatabaseError) as error:
        # Ошибка может быть, если json некорректный, но dumps должен это предотвратить
...
def clear_chat_history(user_id: int, book_id: int) -> bool:
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            conn.commit()
            # rowcount может быть 0, если истории и так не было или строки reading_state нет
            # В любом случае, цель (пустая история) достигнута
            success = True
            # print(f"История чата для user {user_id}, book {book_id} очищена (или была пуста).")
...
# ------------------------------
# Функции получения контента книги
# ------------------------------
...
def get_book_page_content(user_id: int, book_id: int, page: int, page_size_txt: int = 1000) -> str | None:
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            # 1. Получаем информацию о книге из БД (путь, формат, страницы)
            cur.execute(
            ...
            # 2. Проверка выхода за пределы книги
            if total_pages is not None and page >= total_pages:
                 return "Вы достигли конца книги."

            # 3. Чтение контента в зависимости от формата
            if file_format == 'pdf':
                try:
                    with open(full_path, "rb") as book_file:
                        ...
                        if page < len(reader.pages):
                            ...
                            content = extracted_text if extracted_text else f"(Страница {page + 1} пуста или текст не извлечен)"
                             ...
                             content = "Вы достигли конца книги (PDF)." # Доп. проверка
                except Exception as e:
                    content = f"Ошибка чтения PDF файла '{os.path.basename(full_path)}': {str(e)}"

            elif file_format == 'txt':
                try:
                    # Определение кодировки (можно кэшировать)
                    ...
                    with open(full_path, "r", encoding=detected_encoding, errors='replace') as book_file:
                        ...
                        if not read_content:
                            ...
                            if start_pos >= end_pos and end_pos > 0:
                                ...
                                content = read_content # Пустая строка внутри файла
                        else:
                ...
                try:
                    ...
                    with open(full_path, 'rb') as file:
                        ...
                        for page_obj in pdf_reader.pages:
                            ...
                            except Exception: page_text = "" # Игнорируем ошибки отдельных страниц
...
# ------------------------------
# Функции работы с рекомендациями
# ------------------------------
...
def clear_all_data():
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            print(f"Выполнение SQL: {truncate_query.as_string(conn)}") # Показываем запрос для отладки
